...
namespace HamrahFelez.Repository.DataAccess
{
    public static class DataAccessManager
    {
        ...
        {
            get => _connectionString.Value ?? throw new InvalidOperationException("Connection string is not set for the current request.");
        ...
        private static CancellationTokenSource? CreateLinkedCts(CancellationToken callerToken, CancellationToken? timeoutToken)
        {
            List<CancellationToken> tokens = [];

            if (callerToken.CanBeCanceled) tokens.Add(callerToken);

            var requestAborted = _httpContextAccessor?.HttpContext?.RequestAborted ?? default;
            if (requestAborted.CanBeCanceled) tokens.Add(requestAborted);

            if (timeoutToken is { } t && t.CanBeCanceled) tokens.Add(t);
        ...
        public static async Task<IQueryable<T>> GetAllFromViewAsync<T>(
            string select,
            string? where,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!string.IsNullOrWhiteSpace(where)) sql += " " + where.Trim();
        ...
        public static async Task<BaseStoredProcedureResponse> ExecuteOperationalSPAsync(
            string procedure,
            CancellationToken cancellationToken = default,
        ...
        public static async Task<IQueryable<T>> GetAllFromSPAsync<T>(
            string procedure,
            CancellationToken cancellationToken = default,
            params StoredProcedureParameter[] parameters)
        {
